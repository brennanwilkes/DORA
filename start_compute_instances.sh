...
while IFS="\n" read -r worker; do
	...
	echo ssh -o StrictHostKeyChecking=no -i "$KEY" "$USER@$IP" "cd ~/$REPO && touch SCHEDULER_REQUESTED_START && nohup ./worker_study.sh '$i' '$USER' '$IP' '$KEY' >~/worker.out 2>~/worker.err </dev/null &"
